...
import Workers.FFI.Crypto (hmacSha256, secureCompare)
...
-- | 商品マッピングを検索
findProductMapping :: StripeConfig -> String -> String -> Maybe ProductMapping
findProductMapping config stripeProductId stripePriceId =
  head $ filter matchesProduct config.productMappings
  where
    matchesProduct mapping =
      mapping.stripeProductId == stripeProductId ||
      mapping.stripePriceId == stripePriceId
...
instance ChannelAdapter StripeAdapter where
  ...
  getStock adapter (ThingId productId) = do
    pure $ Left $ ConfigurationError "Stripe does not support inventory management"

  setStock adapter productId quantity = do
    pure $ Left $ ConfigurationError "Stripe does not support inventory management"
  ...
  -- Webhook 経由で注文を処理
  processOrders (StripeAdapter config) since = do
